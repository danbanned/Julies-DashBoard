...
export const FIELD_MAP = {
  ...
  eventbrite: {},
  // fespp/parktobroad (confirmed 2026-08-12 against real committed data in
  // data/fespp_events.json and data/parktobroad_events.json) already write
  // canonical field names natively — title, description, category, location,
  // fee, start_date/end_date, event_url, image_url — no rename needed.
  fespp: {},
  parktobroad: {},
  rodinmuseum: {},
  // Barnes' scraper (confirmed 2026-08-12) outputs event_date/venue/cost/
  // featured_image_url instead of the canonical names — without this map
  // every event fails the start_date validation gate even once real data
  // lands. title/event_url/description/category are already canonical.
  barnesfoundation: {
...
// ---------------------------------------------------------------------------
// Settings: Theme / Layout / Seasonal Aesthetic option lists. Three disjoint
// CSS axes — data-jw-theme (geometry only), data-jw-layout (structure only),
// data-jw-season (accent color + the Featured Image hero photo, per the
// settings_layout_aesthetic_corrections.md reversal of the original
// color-stays-constant rule) — each reads its own tokens/attributes only.
// See phase24_layout_theme_settings.md for the original architecture.
// ---------------------------------------------------------------------------
export const THEME_OPTIONS = [
  { value: "original", label: "The Original", blurb: "Today's geometry — pill buttons, soft radius." },
  { value: "ledger", label: "The Ledger", blurb: "Tighter, denser — a working-tool feel for CRM sessions." },
  { value: "trailhead", label: "Trailhead", blurb: "Larger radius, softer glow — the warmest variant." },
];

// Correction pass — 7 options total. `layoutPref` is one value; each surface
// (ViewerApp.js, CrmApp.js) reacts only to the values applicable to it and
// falls back to its own default otherwise (e.g. CrmApp treats "cards" as
// "column"; ViewerApp treats "desk" as "column") — this is what makes each
// option "apply globally within its own domain" without five separate prefs.
export const LAYOUT_OPTIONS = [
  { value: "full", label: "Full Width", blurb: "No max-width clamp — today's pre-Settings original.", scope: "Home/Events/Favorites" },
  { value: "column", label: "The Column", blurb: "Single-feed with a readable max-width. Default.", scope: "Home/Events/Favorites" },
  { value: "desk", label: "The Desk", blurb: "Multi-column grid.", scope: "CRM only" },
  { value: "cards", label: "Card / Block", blurb: "Grid-of-cards instead of a single feed.", scope: "Home/Events/Favorites" },
  { value: "fixed-nav", label: "Fixed Navigation", blurb: "Bottom nav always visible — today's default, now a named choice.", scope: "Home/Events/Favorites" },
  { value: "hidden-nav", label: "Hidden Navigation", blurb: "Bottom nav hidden — the ☰ menu is the only nav.", scope: "Home/Events/Favorites" },
  { value: "featured-image", label: "Featured Image", blurb: "A real photo hero — also where the Seasonal Aesthetic photo shows.", scope: "Home/Calendar/Profile/Badges" },
];

export const SEASON_OPTIONS = [
  { value: "off", label: "Off", blurb: "No seasonal accent or photo." },
  { value: "spring", label: "Spring", blurb: "Sage/green accent." },
  { value: "summer", label: "Summer", blurb: "Amber/gold accent." },
  { value: "fall", label: "Fall", blurb: "Burnt orange/rust accent." },
  { value: "winter", label: "Winter", blurb: "Cool blue accent." },
];
