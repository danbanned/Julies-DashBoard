...
// Renter stages that count as an active contract (Phase 25 — confirmed:
// stage-based, NOT the "Taking on?" flag, since that can be true as early as
// "Contacted", long before any real contract exists).
const RENTER_ACTIVE_CONTRACT_STAGES = ["APPLICATION", "LEASE_SIGNED"];
export function isActiveContract(client) {
  return client.clientType === "BUYER"
    ? ACTIVE_CONTRACT_STAGES.includes(client.stage)
    : RENTER_ACTIVE_CONTRACT_STAGES.includes(client.stage);
}
...
export function parseMoveMonth(raw, year = 2026) {
  ...
  // Anchor at NOON UTC: the column is timestamp-without-tz and read/write can
  // shift by the local offset (±5h) — noon keeps it inside the same day/month
  // regardless, so "August" never renders as "July".
...
// The Google Form's Timestamp arrives as a Sheets SERIAL number (days since
// 1899-12-30, e.g. 46215.95 = 2026-07-12 22:52) OR — if n8n pre-formats it —
// a normal date string. Handle both; return null when unparseable.
export function parseSheetTimestamp(raw) {
  if (raw === null || raw === undefined || raw === "") return null;
  const n = Number(raw);
  if (Number.isFinite(n) && n > 20000 && n < 90000) {
    // Sheets serial → epoch ms (base 1899-12-30 UTC)
...
export function cleanFormRow(row) {
  ...
  const createdAt = parseSheetTimestamp(timestampRaw); // sheet Timestamp → createdAt
  const contactedRaw = String(get("Contacted") || "").trim() || null;
  // "Contacted? = sent" means Julie reached out around the submission time
  const contactedSent = /sent|met with|called|texted|emailed/i.test(contactedRaw || "");
  ...
  return {
    ...
    contactedRaw,
    lastReachedOut: contactedSent ? createdAt : null,
    stage: contactedSent ? "CONTACTED" : "NEW",
    createdAt: createdAt || undefined, // fall back to DB default if unparseable
    formKey: `${String(timestampRaw ?? "").trim()}|${name.toLowerCase()}`,
    rawSource: row,
