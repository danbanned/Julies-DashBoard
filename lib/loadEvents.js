...
export function loadEvents() {
  const rawBySource = {};
  // 22d: a source whose file is missing/malformed degrades to an empty feed
  // so the page never crashes — but that must not happen silently. Track
  // which sources hit this path so it surfaces in the loud per-source log.
  const sourceErrors = {};
  for (const source of SOURCES) {
    try {
      ...
      sourceErrors[source.id] = err.code === "ENOENT" ? "missing" : "malformed";
    }
  }
  const { events, pastEvents, dropped, perSource } = buildEvents(rawBySource, fallbackManifest());
  const chips = neighborhoodsFrom(events); // chips count UPCOMING only
  return { events, pastEvents, dropped, perSource, sourceErrors, chips };
}
