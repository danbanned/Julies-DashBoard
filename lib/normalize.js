...
function normalizeOne(rawEvent, source) {
  ...
  // non-URL strings, and a junk href would render a dead-click card.
  const rawUrl = String(r.event_url || "").trim();
  const event_url = /^https?:\/\//i.test(rawUrl) ? rawUrl : "";
...
// "Upcoming" means upcoming — real Carto data includes program records going
// back to 2018. An event stays if it hasn't ended yet (multi-day events with
// a future end_date stay even if they started in the past).
function todayISO() {
  const d = new Date();
...
export function buildEvents(rawBySource) {
  const dropped = { invalid: 0, hidden: 0, past: 0, deduped: 0 };
  ...
  for (const source of SOURCES) {
    ...
    for (const ev of items) {
      ...
      if (!isUpcoming(ev, today)) { dropped.past++; continue; }
