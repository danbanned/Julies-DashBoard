...
export function buildEvents(rawBySource, fallbackManifest = null) {
  // 22a: "All means all" — every validated event reaches the app. There is
  // no ingestion-time visibility/geography gate anymore (there used to be
  // one, and it silently dropped University City among others). Neighborhood
  // narrowing happens ONLY as a UI-level filter the user opts into.
  const dropped = { invalid: 0, deduped: 0 };
  // 22d: per-source raw counts — a source that silently starts returning 0
  // events (broken scraper, stale n8n cron) looks identical to a source with
  // a legitimately quiet day unless this is logged per-source, not just as
  // one global total.
  const perSource = {};
  ...
  for (const source of SOURCES) {
    ...
    const items = extractArray(raw).map((e) => normalizeOne(e, source, fallbackManifest));
  ...
  return { events, pastEvents, dropped, perSource };
}
